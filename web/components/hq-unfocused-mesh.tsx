...
export function hqMeshBackground(mesh: Partial<HqMesh> = {}): string {
  ...
  layers.push(
    `radial-gradient(ellipse 60% 55% at ${lx}% ${ly}%, ${m.to} 0%, transparent 100%)`,
  );

  layers.push(
    `linear-gradient(${m.angle}deg, ${m.from} 0%, ${m.from} 28%, ${m.to} 100%)`,
  );
...
export function HqUnfocusedMesh({
  ...
  const maskSpec: HqBlurMask | null =
    mask === false ? null : { ...DEFAULT_MASK, ...mask };
  ...
  const driftReach = driftSpec
    ? Math.max(Math.abs(driftSpec.x), Math.abs(driftSpec.y))
    : 0;
